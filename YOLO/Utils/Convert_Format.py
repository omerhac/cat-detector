...
def csv_from_xml(directory, path_name=""):
    # First get all check and xml files from path and its subfolders
    image_paths = GetFileList(directory, ".jpg")
    xml_paths = GetFileList(directory, ".xml")
    result_df = pd.DataFrame()
    if not len(image_paths) == len(xml_paths):
        print("number of annotations doesnt match number of check")
        return False
...
def crop_and_save(
    ...
    and crops the check accordingly
    ...
    df : pd.Dataframe
        ...
        and label
    source_path : str
        Path of source check
    target_path : str, optional
        Path to save cropped check
